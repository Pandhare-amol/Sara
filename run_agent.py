"""
SARA Desktop Control Agent â€” frozen entrypoint.

This is the script PyInstaller freezes into `sara-agent.exe`. It runs the
...
Run (frozen):   sara-agent.exe
Run (dev):      python run_agent.py
Environment:
    SARA_AGENT_HOST   default 127.0.0.1
    SARA_AGENT_PORT   default 8765
    SARA_DATA_DIR     where logs/ is written (default: cwd)
"""
...
def _resolve_data_dir() -> Path:
    data = os.environ.get("SARA_DATA_DIR") or os.getcwd()
    logs = Path(data) / "logs"
    try:
...
def main() -> None:
    ...
    log = logging.getLogger("sara.agent.boot")

    host = os.environ.get("SARA_AGENT_HOST", "127.0.0.1")
    port = int(os.environ.get("SARA_AGENT_PORT", "8765"))
    frozen = getattr(sys, "frozen", False)
    log.info("Starting SARA agent (frozen=%s) on %s:%d", frozen, host, port)
