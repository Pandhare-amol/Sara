...
export async function loadMemories(source: MemorySource = "desktop"): Promise<Memory[]> {
  try {
...
export async function saveMemories(memories: Memory[], source: MemorySource = "desktop"): Promise<void> {
  try {
    const filePath = memoryFileForSource(source);
    await fs.writeFile(filePath, JSON.stringify(memories, null, 2), "utf-8");
    console.log(`[Memory] Saved ${memories.length} memories successfully (${source}).`);
  } catch (error) {
...
export async function processConversationSlice(
  ...
  try {
    ...
    const prompt = `You are Sara's deep cognitive recollection engine. Your task is to analyze the recent conversation piece against previous persistent memories, and output precise update transactions.
