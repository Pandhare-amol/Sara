# SARA Desktop Control Agent

A local Python FastAPI service that gives SARA **JARVIS-style desktop control** â€”
...
> **This agent does NOT modify SARA's UI, personality, or chat system.** It is a pure
> backend tool layer that SARA's existing Node bridge (`server.ts`) calls over HTTP.
...
cd C:\Users\MSI\Desktop\sara-ai-assistant
...
The agent binds to `127.0.0.1:8765`. Then start SARA normally with `npm run dev`.
...
| `saveScreenshot` | Save screenshot to Pictures/SaraScreenshots |
...
- **Power actions** (shutdown, restart, sleep, lock) require a **two-step confirmation token**: SARA must first call `requestPowerAction` (which issues a single-use, 60-second token), ask the user out loud to confirm, then call `executePowerAction` with the token. Without a valid token, the action is refused.
...
```
SARA voice chat (existing, untouched)
        â†“
