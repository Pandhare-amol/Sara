...
Step 1: requestPowerAction(action) -> mints a single-use, short-lived token
        and tells SARA (via the result string) to ask the user to confirm.
...
# Friendly human labels so SARA's prompt-to-confirm reads naturally.
ACTION_LABEL = {
...
def request_power_action(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    SARA calls this first; the returned message instructs her to ask the
