...
SCREENSHOTS_DIR = Path(os.path.expanduser("~")) / "Pictures" / "SaraScreenshots"
...
@register("detectUiElements")
def detect_ui_elements(args: Dict[str, Any]) -> Dict[str, Any]:
    img = _capture()
    try:
        data = _ocr_data(img)
    except ToolError as e:
        return {"result": f"UI detection unavailable: {e.message}", "elements": []}
    elements = []
    for item in data:
        ...
        if conf < 30:
            continue
        x, y, w, h = (int(item.get(k) or 0) for k in ("left", "top", "width", "height"))
        elements.append(
            {
                "text": text,
                "confidence": conf,
    ...
    return {
        "result": f"Detected {len(elements)} visible UI text elements.",
...
def _ocr_data(img):
    try:
        ...
        raise ToolError("OCR unavailable: the 'pytesseract' package is not installed.") from exc
    exe = os.environ.get("TESSERACT_PATH") or _find_tesseract_exe()
    if exe:
        pytesseract.pytesseract.tesseract_cmd = exe
    try:
        ...
        raise ToolError("UI detection failed: " + str(e))
